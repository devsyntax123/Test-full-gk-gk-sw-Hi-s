...
USAGE
-----
All settings are plain variables in the CONFIG block right below the imports
(no command-line arguments). Edit them there, then just run:
...
RESUME BEHAVIOUR
-----------------
Every blob that is successfully moved is recorded in the CSV log (append
mode). On the next run, the script loads the CSV log(s) matching
CSV_LOG_GLOB (in the CONFIG block) and skips any source blob names already
marked "MOVED", so you can safely re-run it repeatedly, each time picking up
the next BATCH_SIZE files, until the source is empty.
"""
...
# ==========================================================================
# CONFIG -- edit these values directly. No command-line arguments are used.
# ==========================================================================

# Azure Storage container name (source and destination prefixes are inside
# this SAME container).
CONTAINER_NAME = "your-container-name"

# Source and destination "folder" prefixes (virtual folders in blob storage).
SOURCE_PREFIX = "call-centre/imarque/cross-sell/result-31-07-2026-filtered/"
DEST_PREFIX = "call-centre/imarque/cross-sell/intermediate-input/"

# Number of blobs to move in this run. Source may contain ~3,00,000 (3 lakh)
# files; default here moves 1,00,000 (1 lakh) at a time. Re-run the script
# to move the next batch -- already-moved blobs are skipped automatically
# (see CSV_LOG_GLOB below).
BATCH_SIZE = 100_000

# Thread pool size for concurrent moves. Tune based on your storage
# account's throughput / request-rate limits.
MAX_WORKERS = 32

# CSV file that records the result of every blob move (append mode).
# A fresh, timestamped filename is used per run by default.
CSV_LOG = f"move_log_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"

# Glob pattern used to find PAST csv logs so already-MOVED blobs are
# skipped when you re-run the script for the next batch.
CSV_LOG_GLOB = "move_log_*.csv"

# Human-readable run log file (progress / warnings / errors).
RUN_LOG = "move_run.log"

# The connection string is read from this environment variable -- set it
# before running:
#     export AZURE_STORAGE_CONNECTION_STRING="<your connection string>"
...
def main():
    ...
    if not connection_string:
        print(f"ERROR: Set the {CONNECTION_STRING_ENV_VAR} environment variable.", file=sys.stderr)
        sys.exit(1)

    logger = build_run_logger(RUN_LOG)
    silence_azure_sdk_logs(RUN_LOG)

    logger.info("=" * 80)
    logger.info("Starting blob move run")
    logger.info(f"Container       : {CONTAINER_NAME}")
    logger.info(f"Source prefix   : {SOURCE_PREFIX}")
    logger.info(f"Dest prefix     : {DEST_PREFIX}")
    logger.info(f"Batch size      : {BATCH_SIZE}")
    logger.info(f"Max workers     : {MAX_WORKERS}")
    logger.info(f"CSV log         : {CSV_LOG}")
    logger.info(f"Run log         : {RUN_LOG}")

    already_moved = load_already_moved(CSV_LOG_GLOB)
    ...
    blob_names = list(
        list_source_blobs(container_client, SOURCE_PREFIX, BATCH_SIZE, already_moved, logger)
    )
    ...
    def worker(src_name: str):
        rel_path = src_name[len(SOURCE_PREFIX):] if src_name.startswith(SOURCE_PREFIX) else src_name
        dest_name = DEST_PREFIX + rel_path
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS, thread_name_prefix="mover") as executor:
    ...
    logger.info(f"CSV log written : {CSV_LOG}")
    if failed_count > 0:
        logger.warning(
            f"{failed_count} blobs FAILED to move. Check '{RUN_LOG}' for details "
