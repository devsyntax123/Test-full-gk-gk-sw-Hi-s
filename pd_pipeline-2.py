...
def extract_images_base64(pdf_bytes: bytes) -> dict:
    ...
    that has raster images. Uses get_image_info(xrefs=True) instead of
    get_images() so images nested inside XObjects/forms are also caught,
    not just top-level embedded images. No files written — pure in-memory.
    """
